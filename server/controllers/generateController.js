...
export const handleGenerate = async (req, res) => {
  try {
    // Expecting toolId in params (from URL) and inputs in body
    const { toolId } = req.params;
    const { inputs } = req.body;

    if (!toolId) {
      return res.status(400).json({ success: false, message: "Missing tool ID" });
    }
    ...
    switch (toolId) {
      case 'script-generator':
        userPrompt = `Write a comprehensive YouTube video script.
        Topic: "${inputs.topic}"
        ...
        Structure the response as Schema Type B with these sections:
        1. Hook (0-30s) - High retention opening
        2. Intro - Context and credibility
        3. Core Content - Break down into 3-5 main points
        4. Engagement Spike - A pattern interrupt or question
        5. Conclusion & CTA
        `;
        break;
      case 'yt-breakdown':
      ...
      case 'yt-transcript':
         ...
         break;
      default:
        userPrompt = `Generate structured content for tool "${toolId}" with inputs: ${JSON.stringify(inputs)}. Follow Schema A or B.`;
    }
